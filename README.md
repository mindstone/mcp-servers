...
- **Reproducible release tarballs.** Releases are currently published manually under the `@mindstone/` npm scope from a wave-lead's machine, behind a human approval gate (see [docs/PUBLISH_APPROVAL_PROCESS.md](docs/PUBLISH_APPROVAL_PROCESS.md)). Provenance attestations are **not** produced for manual publishes, but the tarball shasum is human-verifiable: check out `mindstone/mcp-servers` at the release tag and run `npm pack` — the resulting tarball must match the one on the npm registry. See [MIGRATION.md](MIGRATION.md#what-you-get-on-the-new-scope) for the consumer verification one-liner.
- **OpenSSF Scorecard.** The repo runs the [OpenSSF Scorecard](https://scorecard.dev/viewer/?uri=github.com/mindstone/mcp-servers) weekly via `.github/workflows/scorecard.yml`. The report is generated by a third party from the source tree, so every claim above (action pinning, branch protection, token usage, dependency hygiene) is independently checked rather than just asserted here. The score badge will be added to this README once the first workflow run completes on `main`.
