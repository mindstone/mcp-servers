...
6. Content fetched from external systems is wrapped in `<untrusted-content source="...">` envelopes with close-tag breakout escaping. Do not strip the envelopes or pass raw external strings into model-visible output.
...
- Copy `connectors/_template/`. Do not scaffold from scratch.
...
When bumping a connector, four artefacts change in lockstep:
...
- The Pages catalogue and one-click install blocks must be regenerated by running, from the repo root:
  ...
  Commit any resulting changes under `docs/` and `connectors/<name>/README.md` in the same PR as the version bump.

CI rejects PRs where these drift. The git tag at release time must also match. The `pages.yml` workflow self-heals catalogue drift on `main` by opening an automated regeneration PR — if that PR appears, treat it as a fix-forward signal that a release skipped the regen step locally.
...
- Bump `package.json`, `package-lock.json`, and `server.json` in lockstep, not one at a time, and regenerate the Pages catalogue + install-links blocks (`node scripts/build-catalogue.mjs && node scripts/gen-install-links.mjs`) in the same PR.
