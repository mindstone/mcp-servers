...
| F7 | **MEDIUM** | Audit gap at publish time | `npm audit --audit-level=high --omit=dev` runs in **CI matrix** but **not** in `publish.yml`. Only the `hubspot` connector enforces audit via `prepublishOnly`; the other 23 do not. A newly disclosed CVE between PR-merge and tag-cut would ship without warning. **[UPDATE 2026-06-13: the per-push CI-matrix audit was *removed* (`ci.yml`). It fired on every push regardless of whether the commit changed anything, so a newly published upstream advisory reddened `main` on the next unrelated push — misattributed to whoever pushed. High-severity audit now runs weekly via MindstoneRebel `docs/project/WEEKLY_AUTOMATED_REVIEW.md` and at publish per `PUBLISH_APPROVAL_PROCESS.md`. This *widens* the publish-time gap this finding describes: the only remaining mechanical block is hubspot's `prepublishOnly` — R7 (audit in `release.yml` / `_template` `prepublishOnly`) is now the durable fix and is unblocked.]** |
