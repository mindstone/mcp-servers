...
describe('atomic credential writes', () => {
  it('writes a credential file with 0600 permissions in the parent directory', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'microsoft-shared-atomic-mode-'));
    const filePath = path.join(root, 'credential.json');

    await atomicCredentialWrite(filePath, JSON.stringify({ k: 'v' }));

    expect(fs.readFileSync(filePath, 'utf8')).toBe('{"k":"v"}');
  ...
  it('cleans stale temp files left by a mid-write process death', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'microsoft-shared-atomic-'));
  ...
  it('rejects symlink targets', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'microsoft-shared-symlink-'));
  ...
  it('keeps the original file untouched and cleans temp files when rename fails', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'microsoft-shared-rename-'));
    const filePath = path.join(root, 'credential.json');
