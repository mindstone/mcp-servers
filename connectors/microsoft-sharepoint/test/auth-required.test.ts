...
describe('authenticate_sharepoint', () => {
  ...
  it('emits structured auth_required with sharepoint setup metadata', async () => {
    const result = await client.callTool('authenticate_sharepoint', {});
    expect(result.json).toMatchObject({
      status: 'auth_required',
      user_action: { id: 'microsoft.connect_sharepoint' },
      agent_action: {
        instruction: expect.stringContaining('SharePoint'),
      },
      setupToolName: 'authenticate_sharepoint',
    });
  });

  it('does not emit bridge/internal host vocabulary', async () => {
    const result = await client.callTool('authenticate_sharepoint', {});
...
describe('auth_required envelope on Graph token failure', () => {
  ...
  it('returns the host-orchestrated auth_required envelope when refresh is disabled', async () => {
    const result = await client.callTool('list_sharepoint_sites', { top: 1 });
  ...
  it('makes zero token-refresh POSTs when MICROSOFT_DISABLE_REFRESH=1', async () => {
    await client.callTool('list_sharepoint_sites', { top: 1 });
