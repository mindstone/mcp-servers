 ...
 *
 * Provides Microsoft 365 SharePoint integration via Model Context Protocol —
 * list/search/get SharePoint sites, libraries, pages, lists, and items;
 * upload/move/copy/delete files and list items; and emit host-orchestrated
 * SharePoint scope-auth (`authenticate_sharepoint`) through Microsoft Graph.
 *
 ...
 *                             Defaults to "Microsoft365SharePoint".
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(
    'Microsoft 365 SharePoint MCP Server. Configure MS_CONFIG_DIR + MS_CLIENT_ID and run over stdio from an MCP host.',
  );
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  console.error('[microsoft-sharepoint-mcp] running on stdio');
}
