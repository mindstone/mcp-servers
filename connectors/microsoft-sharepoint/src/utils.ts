 ...
 *
 * Mirrors Slack/Google Workspace cohort precedent. SharePoint keeps its own
 * setup tool so the host can request incremental Sites.Read.All consent.
 */
export function buildAuthRequiredResponse(): Record<string, unknown> {
  return {
    status: 'auth_required',
    user_action: { id: AUTH_USER_ACTION_ID },
    agent_action: {
      instruction:
        'Grant SharePoint permissions to continue. The user will be redirected to the Microsoft consent flow.',
    },
 ...
 * Mirrors bundled microsoft-sharepoint `errorResult` parity: `isError: true`
 * plus `{ ok: false, error, action_required, next_step }` payload.
 */
 ...
 * microsoft-sharepoint `successResult` parity (no `ok: true` wrapper).
 */
 ...
 * status/code metadata. Map those messages to token_expired so SharePoint still
 ...
 * a JSON string. Graph 401/refresh failures translate to
