...
/**
 * Calendar does NOT declare an `authenticate_microsoft_account` tool of its
 * own — the cohort decision is that Mail owns the M365 OAuth surface and the
 * host's `resolveMicrosoftAuthServerId()` routing redirects Calendar's setup
 * calls to Mail's running instance. What Calendar MUST still do is emit the
...
describe('auth_required envelope on Graph token failure', () => {
  ...
  it('returns the host-orchestrated auth_required envelope when refresh is disabled', async () => {
    const result = await client.callTool('list_events', { top: 1 });
  ...
  it('makes zero token-refresh POSTs when MICROSOFT_DISABLE_REFRESH=1', async () => {
    await client.callTool('list_events', { top: 1 });
...
describe('auth_required envelope on refresh failure', () => {
  ...
  it('returns auth_required when refresh attempt fails', async () => {
    ...
    expect(state.refreshCalls).toBeGreaterThanOrEqual(1);
  });
});
