#!/usr/bin/env node
/**
 * Microsoft 365 Outlook Calendar MCP Server.
 *
 * Provides Microsoft 365 calendar integration via Model Context Protocol —
 * list, get, create, update, delete, respond to events, check free/busy, and
 * list calendars through the Microsoft Graph API.
 *
 ...
 *                             Defaults to "Microsoft365Calendar".
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(
    'Microsoft 365 Outlook Calendar MCP Server. Configure MS_CONFIG_DIR + MS_CLIENT_ID and run over stdio from an MCP host.',
  );
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  console.error('[microsoft-calendar-mcp] running on stdio');
}
