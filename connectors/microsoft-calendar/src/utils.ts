 ...
 *
 * Mirrors Slack/Google Workspace cohort precedent. Calendar reuses the cohort's
 * Microsoft 365 OAuth surface owned by `mcp-server-microsoft-mail`; the host's
 * routing layer redirects setupToolName calls to Mail's running instance.
 */
 ...
 * Mirrors bundled microsoft-calendar's `errorResult` parity: `isError: true`
 ...
 * microsoft-calendar's `successResult` parity (no `ok: true` wrapper).
 */
...
export function buildErrorResponse(err: unknown): CallToolResult {
  if (err instanceof CalendarBusinessError) {
