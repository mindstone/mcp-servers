...
- Structural keys (`id`, `uuid`, `url`, `shared_link`, …) stay raw only while the value still validates as the identifier/URL it claims to be; anything else is enveloped like other external text. The validators are now strict shapes rather than permissive charsets: identifiers must be a UUID, a dense alphanumeric token, or short separator-joined segments (a word-separated instruction phrase such as `SYSTEM-ignore-all-previous-instructions` is enveloped), and URLs stay raw only for `https:` links on PandaDoc-owned hosts (any other URL is enveloped, since instruction-like path/query text is indistinguishable from a valid URL string).
- `upload_document` validates and reads the workspace file as one race-resistant operation: canonical containment, a single `O_NOFOLLOW` open (a post-validation leaf-symlink swap fails with ELOOP instead of being followed), `fstat`-based type/size checks on the opened inode, an ancestor-swap re-resolution bound to that inode, and a bounded read through the descriptor. The previous `fstat`+`stat` compare could not catch a leaf swapped for an escaping symlink, because both observations saw the same outside inode.
...
- `create_document_from_url` rejects loopback, link-local, private-range, and reserved literal hosts (IPv4 and IPv6, incl. IPv4-mapped), credential-bearing URLs, and internal hostname suffixes before any network call. It also DNS-resolves the host and refuses when any answer is non-public, then follows the redirect chain itself (bounded hops, every hop re-validated) and hands PandaDoc only the terminal URL — a public URL that redirects internally is refused instead of dereferenced. (DNS-rebinding TOCTOU between the connector's lookup and PandaDoc's own fetch remains vendor-side and is documented in `src/tools/url-safety.ts`.)
- Identifier/URL strict-shape validation now reaches every output path: the compact projections (`list_documents`, `list_templates`, `list_document_folders`, `list_content_library_items`, `get_document_status`, `get_document_details`, `send_document`, and the create/upload responses) validate top-level `id`/`uuid` values before keeping them raw, and `create_document_session` fails closed when the returned session id is not identifier-shaped instead of interpolating it raw into the signing URL.
- `get_content_library_item_details`, `list_content_library_items`, and `list_document_folders` now project known response fields explicitly (fail-closed, matching the document tools), so fields outside the known shape — including any PandaDoc adds later — never reach the model unsanitized.
- `upload_document` parses a 2xx response body fail-closed: a non-JSON success body yields a generic INVALID_RESPONSE error instead of a runtime parse message that can echo body fragments.
- `create_document_from_url` envelopes the rejected-URL error reason, which can embed an attacker-chosen redirect URL or host.
- `download_document` bounds the document-id-derived portion of the download filename, so an over-long id cannot surface a raw ENAMETOOLONG through the generic error path.
...
- Open-ended request bodies now validate real PandaDoc shapes fail-closed instead of `z.unknown()`/generic-JSON pass-through: `fields` entries must be `{ "value": scalar }` objects, `metadata` a string-keyed scalar map, and pricing-table `options`/`data`/`custom_fields` follow the documented column/adjustment structures (`{ "type": "percent"|"absolute", "value": number }`, ISO-style 3-letter `currency`). Recipient `email` fields on `create_document_from_template`, `upload_document`, and `create_document_from_url` must be valid email addresses.
