...
- Re-sync the vendored envelope helper with the hardened canonical: attribute-bearing close tags and spoofed open tags escaped.
...
- Local media reads no longer stat-then-reopen by pathname: the validated file is opened once with `O_NOFOLLOW`, `fstat`-verified as a regular file within the size limit, then re-verified post-open (the path must still canonically resolve inside the workspace AND name the same inode as the opened descriptor) and read through that same descriptor — closing both leaf-symlink and ancestor-directory swap races between validation and read.
- `download_kling_video` now writes to a fresh `fs.mkdtempSync` staging directory (0700, unpredictable name) inside the canonical download root and places the finished file with a metadata-only operation: a hard link that fails `EEXIST` on any pre-existing path (default no-clobber), or — with `overwrite: true` — an atomic rename over the validated target after a device+inode identity re-check. A parent directory or target swapped mid-download is refused observably, and a symlink or hardlink planted at the target is never followed or written through (previously a planted hardlink could be clobbered via `O_TRUNC`).
- The rate-limit error message no longer interpolates the vendor's `Retry-After` header verbatim; only a plain numeric seconds value is honoured (anything else falls back to the default wait text), so vendor-controlled header text cannot reach model-visible output unenveloped.
