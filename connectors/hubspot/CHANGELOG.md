...
- Hardened the envelope walker after adversarial review: every value inside a record's `properties` bag is now enveloped regardless of property name (custom property names are tenant-defined, so a free-text property named `status`/`type`/`email` could previously bypass the envelope; record IDs still round-trip via the top-level `id`). Per-surface exceptions are path-scoped rules pinned to the documented response shapes instead of key-name sets applied at every nesting level, object keys are defanged against close-tag breakouts, and the walk fails closed beyond a depth/node budget.
...
- The sales-email-read scope check's in-flight introspection is now keyed per access token like the memoised verdict — a concurrent token rotation/reconnect could previously await another token's check and cache its verdict under the wrong token, suppressing (or forging) the redaction warning.
...
- `get_contact_engagements` now reports partial failures in model-visible `notes` — a failed association lookup names the engagement types missing from the timeline, and omitted per-engagement fetches are counted. Previously the gaps were only logged, so an incomplete timeline could look complete.
