 ...
 This link should be displayed to the user.",
  {
    ...
    currencyCode: z.nativeEnum(CurrencyCode).describe("Optional invoice currency, such as USD. Only use a currency enabled in the connected Xero organisation.").optional(),
  },
  async ({ contactId, lineItems, type, reference, date, currencyCode }) => {
    const xeroInvoiceType = type === "ACCREC" ? Invoice.TypeEnum.ACCREC : Invoice.TypeEnum.ACCPAY;
    const result = await createXeroInvoice(contactId, lineItems, xeroInvoiceType, reference, date, currencyCode);
    if (result.isError) {
      return {
        content: [
          {
