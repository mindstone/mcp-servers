...
function createGetHistoryTool(def: HistoryEntityDefinition) {
  return CreateXeroTool(
    ...
    `Retrieve the history (audit trail) for a specific ${def.entity} in Xero. Returns history records including dates, users, details, and changes.`,
    {
...
function createAddNoteTool(def: HistoryEntityDefinition) {
  return CreateXeroTool(
    `add-${def.entity}-note`,
    `Add a note to a specific ${def.entity} in Xero. Notes appear in the entity's history/audit trail. Notes are immutable once created.`,
    {
      [def.idParam]: z.string().describe(def.idDescription),
      note: z
        .string()
