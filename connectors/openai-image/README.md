...
- Writes are open-then-verify: after the fence approves the output directory's canonical path (re-checked in full once `mkdir` has materialised it), the connector opens the output file with `wx` (a fresh, descriptor-pinned inode), re-canonicalises the directory before any bytes flow, and writes through the descriptor. A directory swapped for a symlink between validation and write fails closed with `WORKSPACE_FENCE_VIOLATION`, and the misplaced empty file is cleaned up.
