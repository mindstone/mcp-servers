 ...
 *
 * The helper must be the canonical STRONG family (kept byte-for-byte in sync
 * with `connectors/_template/src/untrusted-content.ts`): every
 * case/whitespace variant of the close tag — including newline,
 * carriage-return, form-feed and vertical tab before `>` — must be neutralised
 * so wrapped attacker-controlled text cannot terminate the envelope early.
 */
...
const SOURCE = 'openai-image:test';
...
describe('wrapUntrusted', () => {
  it('wraps a plain string in a single envelope', () => {
    expect(wrapUntrusted('hello', SOURCE)).toBe(
      '<untrusted-content source="openai-image:test">hello</untrusted-content>',
    );
  });
...
describe('wrapUntrustedJsonStrings', () => {
  it('wraps nested string values, array items, and object keys', () => {
    ...
    expect(arr[0]).toBe(
      '<untrusted-content source="openai-image:test">v1</untrusted-content>',
    );
