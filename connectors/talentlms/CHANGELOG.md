...
- **bridge**: The host bridge state file is now opened once, `fstat`-verified through the descriptor (regular file, size-capped), read through that same descriptor, and shape-validated (integer port 1-65535, non-empty token) before use, so a swapped, crafted, or malformed state file can no longer redirect bridge traffic or pass unchecked. Read/parse failures log an explicit stderr warning instead of silently degrading to "no bridge", and malformed bridge HTTP responses surface a connector-authored error instead of a raw parser error.
- **bridge**: Bridge HTTP responses are now validated with a Zod schema instead of a bare cast — a truthy non-boolean `success` (or any unexpected shape) fails closed and no longer stores the submitted credentials — and the bridge-supplied `warning`/`error` free-text fields are enveloped as `<untrusted-content>` at the boundary before they can reach model-visible output. Bridge request failures now surface a fixed connector-authored message; the raw exception text stays on stderr.
