...
/**
 * Creates a fresh MCP test client connected to the Browserbase connector server.
 *
 * Wraps the shared test-harness createInMemoryTestClient with module reset
 * logic: vi.resetModules() + dynamic import ensures client.ts re-evaluates
...
export async function createTestClient(options: TestClientOptions = {}): Promise<McpTestClient> {
  ...
  // client.ts captures BROWSERBASE_API_KEY at import time, so it must be set first.
