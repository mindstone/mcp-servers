...
/**
 * Reject localhost, IPv6 loopback, and private/loopback/link-local IPv4
 * hostnames. Shared by validateHostname and validatePublicWebUrl.
 */
...
/**
 * Validate a URL for the fetch_url tool. Browserbase fetches the URL
 * server-side, so plain http:// targets are legitimate (unlike
 * validateHostname, which is HTTPS-only) — but private/loopback hosts are
 * still rejected so the tool cannot be used to probe internal networks
 * through Browserbase's egress.
 *
 * @returns The validated URL string, unchanged.
 * @throws ConnectorError if the URL is malformed or targets a non-public host.
 */
export function validatePublicWebUrl(input: string): string {
  const trimmed = input.trim();
  let parsed: URL;
  try {
    ...
    throw new ConnectorError(
      `URL could not be parsed: ${trimmed}`,
      'INVALID_URL',
      'Provide a full URL including the scheme, e.g. "https://example.com/page".',
    );
  }
  const scheme = parsed.protocol.toLowerCase();
  if (scheme !== 'https:' && scheme !== 'http:') {
    throw new ConnectorError(
      `Scheme "${scheme.replace(/:$/, '')}" is not allowed. Only HTTP and HTTPS are permitted.`,
      'INVALID_URL',
