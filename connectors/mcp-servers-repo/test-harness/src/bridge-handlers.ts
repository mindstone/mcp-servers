 ...
 *
 * The bridge is used by connectors to communicate with the host app (Rebel)
 * for credential configuration flows.
 *
 * - Requests without a Bearer token get 401.
 * - With a valid token, returns `{ success: true }` (or custom data).
 *
 * @param port - The bridge port number.
 * @param options - Optional error/success customization.
 * @returns MSW HTTP handlers to register via `mswServer.use(...)`.
 */
