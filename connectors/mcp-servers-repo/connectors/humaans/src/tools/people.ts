...
export function registerPeopleTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        `Get the current authenticated user's profile from Humaans.
...
- list_humaans_time_away: Filter by your personId`,
  ...
  server.registerTool(
    ...
    {
      description:
        `List employees from Humaans HR.
...
- list_humaans_job_roles: Pass personId to see job role history`,
    ...
    withErrorHandling(async (args) => {
      ...
      if (args.team) params.set('teams', args.team);
  ...
  server.registerTool(
    ...
    {
      description:
        `Get full employee profile from Humaans by their ID.
