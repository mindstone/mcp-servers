...
function formatGenerationResult(response) {
    return {
        ok: true,
...
export function registerGenerationTools(server) {
    server.registerTool('recraft_get_me', {
        ...
        const response = await recraftFetch('/users/me');
        return JSON.stringify({ ok: true, user: response });
    }));
    server.registerTool('recraft__generate_image', {
        ...
        if (!isConfigured())
            return noApiKeyError();
        const response = await recraftFetch('/images/generations', {
            method: 'POST',
            body: JSON.stringify({
                prompt: args.prompt,
                size: args.size,
    ...
    server.registerTool('recraft__image_to_image', {
        ...
        const response = await recraftFetch('/images/imageToImage', {
            method: 'POST',
            body: form,
        });
        return JSON.stringify(formatGenerationResult(response));
    }));
    server.registerTool('recraft__creative_upscale', {
        ...
        if (!isConfigured())
            return noApiKeyError();
        const form = await buildMultipartForm({ response_format: args.responseFormat }, [{ fieldName: 'file', input: args.imageURI }]);
        const response = await recraftFetch('/images/creativeUpscale', {
            method: 'POST',
            body: form,
        });
        return JSON.stringify(formatGenerationResult(response));
    }));
    server.registerTool('recraft__replace_background', {
        ...
        const response = await recraftFetch('/images/replaceBackground', {
            method: 'POST',
            body: form,
        });
        return JSON.stringify(formatGenerationResult(response));
    }));
    server.registerTool('recraft_remove_background', {
        ...
        if (!isConfigured())
            return noApiKeyError();
        const form = await buildMultipartForm({ response_format: args.responseFormat }, [{ fieldName: 'file', input: args.imageURI }]);
        const response = await recraftFetch('/images/removeBackground', {
            method: 'POST',
            body: form,
        });
        return JSON.stringify(formatGenerationResult(response));
    }));
    server.registerTool('recraft_vectorize_image', {
        ...
        if (!isConfigured())
            return noApiKeyError();
        const form = await buildMultipartForm({ response_format: args.responseFormat }, [{ fieldName: 'file', input: args.imageURI }]);
        const response = await recraftFetch('/images/vectorize', {
            method: 'POST',
            body: form,
        });
        return JSON.stringify(formatGenerationResult(response));
