...
export function registerStylesTools(server) {
    server.registerTool('recraft_list_styles', {
        ...
        const response = await recraftFetch(`/styles?limit=${args.limit}`);
        return JSON.stringify({ ok: true, styles: response });
    }));
    server.registerTool('recraft__create_style', {
        ...
        if (!isConfigured())
            return noApiKeyError();
        const form = await buildMultipartForm({ style: args.style }, await Promise.all(args.imageURIs.map(async (input, index) => ({ fieldName: `file${index + 1}`, input }))));
        const response = await recraftFetch('/styles', {
            method: 'POST',
            body: form,
        });
