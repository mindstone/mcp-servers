...
server.registerTool("get_cutting_guide", {
...
// Tool: list_fruits
// =============================================================================
server.registerTool("list_fruits", {
...
Returns:
  List of supported fruit names with emoji.`,
    ...
    const lines = FRUIT_DATABASE.map((f) => `${f.emoji} **${f.name}** — ${f.technique} (${f.difficulty})`).join("\n");
    return {
        content: [{
                type: "text",
...
server.registerTool("compare_fruits", {
