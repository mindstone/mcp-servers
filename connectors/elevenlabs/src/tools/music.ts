...
COST: FREE — no credits consumed.`,
    ...
    withErrorHandling(async (args) => {
      ...
      // Considered exclusion (invariant #6 review, 260709): the composition
      // plan is generated by the ElevenLabs model FROM THE USER'S OWN PROMPT —
      // no third party authors any of it — and it must round-trip verbatim
      // into generate_music_from_plan's strict schema, which envelopes would
      // break. Deliberately NOT wrapped; same class as design_voice preview
      // text. Flagged for the release security review.
      return JSON.stringify({
        ok: true,
