...
- **email-imap**: `email_set_flags` flag keywords must now match a charset allowlist (`\?[A-Za-z0-9_$.-]+`) instead of only rejecting envelope-marker shapes — IMAP atom-specials (spaces, parens, braces, quotes, list wildcards, CR/LF) are refused before the value reaches the command compiler, so a keyword can never alter the STORE command's structure regardless of how the underlying client serialises ATOM values.
