 ...
 *
 * Mirrors Slack/Google Workspace cohort precedent. Files reuses the cohort's
 ...
 * Mirrors bundled microsoft-files' `errorResult` parity: `isError: true`
 ...
 * microsoft-files' `successResult` parity (no `ok: true` wrapper).
 */
...
export function buildErrorResponse(err: unknown): CallToolResult {
  if (err instanceof FilesBusinessError) {
