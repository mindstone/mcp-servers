#!/usr/bin/env node
/**
 * Microsoft 365 OneDrive Files MCP Server.
 *
 * Provides Microsoft 365 OneDrive integration via Model Context Protocol —
 * list, get, download, search, upload, create folder, delete, move, copy,
 * recent, shared, share, and read text files through the Microsoft Graph API.
 *
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(
    'Microsoft 365 OneDrive Files MCP Server. Configure MS_CONFIG_DIR + MS_CLIENT_ID and run over stdio from an MCP host.',
  );
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  console.error('[microsoft-files-mcp] running on stdio');
}
