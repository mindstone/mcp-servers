...
// Real (minimal) Office Open XML containers served as binary content for
// read_document tests; generated by a checked-out one-off script, committed
// as binary fixtures.
...
function documentMetadata(id: string) {
  switch (id) {
    case 'item-docx':
      return { id, name: 'report.docx', size: docxFixture.length, file: { mimeType: DOCX_MIME } };
    case 'item-pptx':
      return { id, name: 'deck.pptx', size: pptxFixture.length, file: { mimeType: PPTX_MIME } };
    case 'item-pdf':
      return { id, name: 'paper.pdf', size: 2048, file: { mimeType: 'application/pdf' } };
    case 'item-corrupt':
      return { id, name: 'broken.docx', size: 32, file: { mimeType: DOCX_MIME } };
    default:
      return null;
  }
}
...
export function createMockApi(): { handlers: HttpHandler[]; state: MockApiState } {
  ...
  const handlers: HttpHandler[] = [
    ...
    // /me/drive/items/{id}/content (GET) — read_text_file / read_document content by id
    http.get(
      ...
      async ({ request }) => {
        ...
        if (url.pathname.includes('/items/item-corrupt/')) {
          return HttpResponse.text('this is not a zip archive');
        }
        return HttpResponse.text('Hello from text file.');
      },
    ),
    ...
    http.get(
      ...
      async ({ request }) => {
        ...
        if (select.includes('file,folder')) {
          const itemId = /\/items\/([^/]+)$/.exec(url.pathname)?.[1] ?? '';
          const doc = documentMetadata(itemId);
          if (doc) return HttpResponse.json(doc);
          return HttpResponse.json(sampleTextMetadata);
        }
        return HttpResponse.json(sampleFile);
      },
