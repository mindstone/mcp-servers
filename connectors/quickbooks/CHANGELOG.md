...
- Enable production writes by default: the QB_ALLOW_PROD_WRITES gate is removed; invocation gating is the host tool-approval layer's job.
...
### Security
- The wholesale-wrapped tools (`query_quickbooks`, `get_quickbooks_entity`, `get_quickbooks_report`) now envelope JSON object keys as well as values: the vendored `untrusted-content.ts` helper is restored byte-for-byte to the canonical `_template` reference after drifting to wrap values only, so an API-controlled object key can no longer reach model output unenveloped.
...
- `sanitizeQboEntity` no longer returns strings reached as array elements (or bare top-level payloads) raw. With no object key to evaluate, the structural predicate cannot apply, so such strings are enveloped unconditionally — the same behavior `wrapUntrustedJsonStrings` already had. This closes a bypass where a compromised API returning e.g. `{"PrivateNote": ["</untrusted-content> …"]}` could smuggle a live close tag to model output past the deny-by-default sanitizer.
