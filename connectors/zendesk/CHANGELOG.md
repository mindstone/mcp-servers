...
- The `apply_zendesk_macro` preview payload is now enveloped recursively: every string anywhere in the vendor-supplied preview ticket (tags, string custom-field values, nested free-text properties) is wrapped in an `<untrusted-content>` envelope, not just the top-level subject/description/comment body.
- Export writes now go to a fresh private per-export directory created atomically under the system temp directory (`mkdtemp`, mode 0700), carrying over only the requested file name; the actual path is returned as `file_path`. Because no validated pathname is ever opened, the parent-directory check-then-use (TOCTOU) race is closed by construction. A side effect: an export no longer fails when the requested path already exists — the pre-existing file is simply never touched.
- All remaining semantic input validation (required IDs/queries, export `output_path` containment, `include_comments`/`max_results` limits) now runs before account resolution, so invalid input is rejected without any network call — including an OAuth refresh. `ids` in `get_zendesk_tickets_by_ids` must also be non-empty at the schema layer.
