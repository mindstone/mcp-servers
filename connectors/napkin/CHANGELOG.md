...
- **napkin**: Enforce the "machine identifiers stay raw" carve-out instead of assuming it. Response validation now constrains codes, request/visual/style IDs, and color mode to a bounded identifier charset (no whitespace or markup) and file URLs to HTTPS with no whitespace, quotes, or angle brackets — a hostile value fails closed as `INVALID_RESPONSE` rather than reaching the model raw. `generated_files[].visual_query` is vendor-echoed free text, not an identifier, so it is now wrapped in an `<untrusted-content>` envelope like the other free-text fields.
