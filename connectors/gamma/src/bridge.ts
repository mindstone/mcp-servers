...
const loadBridgeState = (): BridgeState | null => {
  ...
  // non-regular or oversized file is refused. Failures are observable on
  // stderr rather than silently treated as "no bridge".
  let fd: number;
  try {
    fd = fs.openSync(BRIDGE_STATE_PATH, 'r');
  } catch {
    console.error('[gamma] Bridge state file is not readable; continuing without the bridge.');
    return null;
  }
  try {
    const stat = fs.fstatSync(fd);
    if (!stat.isFile() || stat.size > MAX_BRIDGE_STATE_BYTES) {
      console.error('[gamma] Bridge state file is not a regular file of expected size; ignoring it.');
      return null;
    }
    ...
    if (
      ...
      console.error('[gamma] Bridge state file has an unexpected shape; ignoring it.');
      return null;
    }
    return { port: candidate.port as number, token: candidate.token };
  } catch {
    console.error('[gamma] Bridge state file could not be parsed; continuing without the bridge.');
    return null;
  } finally {
    try {
      fs.closeSync(fd);
