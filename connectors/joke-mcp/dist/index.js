...
function getDailyJoke(category) {
...
server.tool("joke_get_daily", "Get today's joke — same joke all day, changes daily. Great for morning automations.", {
    category: z
...
server.tool("joke_get_random", "Get a random joke — different every time you call it.", {
    category: z
        .enum(["programming", "dad", "general", "ai", "design"])
        .optional()
        .describe("Filter jokes by category. Leave blank for any category."),
...
server.tool("joke_list_categories", "List all available joke categories.", {}, async () => {
    const categories = ["programming", "dad", "general", "ai", "design"];
