...
- **Untrusted-content envelopes.** Every allowlisted string returned from Workday (names, titles, emails, statuses, dates — including values that arrive in an unexpected shape) is wrapped in `<untrusted-content source="workday">` envelopes so the model treats them as data, not instructions. `id` and `href` stay raw so they can be passed back into later tool calls.
