...
- Drop the over-broad cloud-platform scope from all gcloud sites and bound stderr error logging.
...
- Duplicate report header names (two nameless headers, or a dimension and metric sharing a name) are disambiguated with a ` (2)`-style suffix instead of silently overwriting each other's columns in formatted rows.
- Supplying an external abort signal to an API call no longer disables the 30s request timeout; external aborts are forwarded into the connector's own controller.
...
- The vendor-supplied `error.status` string only becomes the structured error `code` when it matches Google's enum shape (e.g. `PERMISSION_DENIED`); anything else falls back to the numeric HTTP status, so arbitrary vendor text never reaches `code` raw.
- The remaining raw external-text sinks are enveloped: data-stream `webStreamData`/app stream-data blobs (the `defaultUri` is property-editor-authored), the global site tag `snippet`, key-event string default values, custom calculated-metric `expression` fields, and metadata compatibility-name lists.
- The metadata user-authored gate no longer fails open on the vendor-supplied `customDefinition` flag alone: fields with recognised custom `apiName` prefixes (`customUser:`/`customItem:`/`customEvent:`/`calculatedMetric:`) are enveloped even when the flag is absent.
- Resource IDs (property, account, audience, audience export, report task, data stream) are constrained to the `[A-Za-z0-9_-]` charset before URL path interpolation, rejecting path-traversal/query-fragment characters with `INVALID_RESOURCE_ID` whether they arrive via tool arguments or vendor responses.
