...
const ROYAL_FAMILY: RoyalMember[] = [
...
Returns:
  ...
  async () => {
    const lines = ROYAL_FAMILY.map((m, i) =>
      `${i + 1}. **${m.name}** — ${m.title} (born ${m.born})`
    ).join("\n");
    return {
      content: [{
        type: "text",
...
Example:
  ...
  async (params) => {
    ...
    const matches = ROYAL_FAMILY.filter(m =>
...
function formatMember(m: RoyalMember): string {
  return [
    `**${m.name}**`,
    `Title: ${m.title}`,
    `Born: ${m.born}`,
