...
### Fixed
- **Single-call prompt checks now complete before the call is created**: `create_phone_call` and `create_web_call` previously raced the dynamic-variable prompt check against the create request (`Promise.all`), so the call could be initiated before validation finished and any warning arrived too late to act on. The check now resolves BEFORE the POST. The check remains fail-open but observable: a lookup failure surfaces an explicit "prompt check could not run" warning and never blocks the explicitly-requested call.
- **Prompt validation is version-aware and never silently indeterminate**: pre-call validation now keys on the effective `(agent_id, agent_version)` — the same agent ID at two published versions can carry different prompts, so each version's agent is fetched (`GET /get-agent/{id}?version=…`) and validated against its own tasks' variable keys. `create_batch_call` groups tasks per distinct pair; the phone-number binding's `agent_version` is used when no override is given. When the effective prompt cannot be identified at all (the agent lookup returns no `response_engine.llm_id`, e.g. a conversation-flow engine or unpublished version), the check now emits an explicit warning instead of silently skipping validation.
- **`create_batch_call` pre-call validation hardened**: the dynamic-variable prompt check now completes BEFORE `POST /create-batch-call` (previously the two raced under `Promise.all`, so a campaign could be created before validation finished). Tasks are grouped by effective agent — the phone number's default outbound agent plus each distinct per-task `override_agent_id` — and each agent's live prompt is validated against the variable keys its own tasks use, with per-agent warnings. A failed prompt check no longer silently skips: it surfaces an explicit "prompt check could not run: <reason>" warning while still placing the explicitly-requested batch.
...
### Security
- **HTTP statusText and malformed-response bodies can no longer inject raw upstream text**: non-JSON error responses previously fell back to raw `response.statusText` in model-visible error output, and a malformed 2xx body rethrew Node's JSON parse error, which embeds the start of the response body. Both were upstream-controlled text reaching the model unenveloped. statusText is now wrapped in the same `<untrusted-content source="retell:error">` envelope as JSON error details, and malformed successful responses fail with a trusted generic `INVALID_RESPONSE` error that carries no body bytes.
- **Knowledge-base uploads fail closed on same-inode mutation**: a writer that truncates, overwrites, or grows an approved upload file mid-read keeps the same inode, which the open-time dev/ino identity check cannot catch. The reader now fstats the descriptor AGAIN after the read and fails closed with `FILE_CHANGED_DURING_READ` on any size/mtime move, and re-enforces the 50MB bound on the bytes actually read.
- **Knowledge-base file uploads are open-then-validate (TOCTOU)**: the workspace sandbox previously validated a file's canonical path and then `statSync`/`readFileSync`'d that pathname, leaving a check-then-use window where a concurrent writer could swap in a different file between check and read. The upload reader now opens a descriptor once, confirms via `fstat` that the opened inode (dev/ino) is the file the sandbox validated, re-checks regular-file and 50MB bounds on the descriptor, and reads through it. A mid-flight swap or deletion fails closed with `FILE_CHANGED_DURING_READ` before any bytes leave the workspace.
- **Upstream API error text is enveloped**: Retell's `error_message`/`detail`/`message` error fields were interpolated raw into model-visible error output. They are now wrapped in `<untrusted-content source="retell:error">` envelopes with close-tag breakout escaping, consistent with every other external-text field (AGENTS.md security invariant #6). The structured `code`/`resolution` error contract is unchanged.
