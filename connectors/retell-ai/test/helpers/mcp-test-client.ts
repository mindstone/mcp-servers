...
/**
 * Creates a fresh MCP test client connected to the Retell AI connector server.
 *
 * Wraps the shared test-harness createInMemoryTestClient with Retell-specific
 * module reset logic: vi.resetModules() + dynamic import ensures client.ts
...
export async function createTestClient(options: TestClientOptions = {}): Promise<McpTestClient> {
  ...
  // client.ts captures RETELL_API_KEY at import time, so it must be set first.
  for (const [key, value] of Object.entries(env)) {
    vi.stubEnv(key, value);
  }

  // Reset module registry so client.ts re-evaluates with new env
  vi.resetModules();
