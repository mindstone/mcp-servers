...
function formatTranscriptAsText(entries) {
    return entries
        .map((entry) => {
...
async function fetchMeetingSummary(recordingId) {
    try {
...
async function fetchTranscript(recordingId) {
    try {
        const response = await fathomFetch(`/recordings/${recordingId}/transcript`);
 ...
 */
function buildRebelSourceDocument(meeting, transcript, summary) {
...
export function registerSyncTools(server) {
    server.registerTool('sync_fathom_meetings_to_rebel', {
        description: `Sync Fathom meeting transcripts to Rebel as source memories.
...
Returns a summary of synced meetings with success/failure per meeting.`,
        ...
        if (args.since)
            params.set('created_after', args.since);
        const allMeetings = [];
        let cursor;
        do {
            if (cursor)
            ...
            if (allMeetings.length >= args.max_meetings)
                break;
        } while (cursor);
