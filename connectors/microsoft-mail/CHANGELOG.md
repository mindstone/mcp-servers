...
- A non-2xx response on the `download_attachment` content or metadata stream now surfaces the upstream error detail (enveloped as untrusted content) instead of degrading to a bare `Microsoft Graph API error (HTTP <status>)` — or, for 403s, an empty envelope: the Graph SDK throws these errors with the response body still an unread stream, which is now drained (with a hard byte cap) into text before classification, also releasing the underlying socket.
...
- The `download_attachment` metadata `$select` now explicitly requests `@odata.type`: the not-a-file-attachment guard reads that annotation, so the request must ask for it rather than rely on the server's default projection (a server omitting it would have silently failed the guard open).
