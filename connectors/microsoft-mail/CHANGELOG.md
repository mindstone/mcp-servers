...
- `download_attachment` pins the validated attachment directory behind an open file descriptor and addresses the create, the post-write verification, and any cleanup relative to that descriptor (`/proc/self/fd/<fd>/<name>`), so a directory swap at any point in the sequence can no longer place bytes outside `MCP_WORKSPACE_PATH` or make cleanup delete anything but the file the connector created; a rename mid-write fails closed instead of reporting a dangling `savedTo`.
- `download_attachment` no longer has a path-based write fallback. This descriptor-relative mechanism is only reachable through Linux `/proc/self/fd` (Node exposes no openat-style API); a path-based create re-resolves its parent chain at syscall time, so a swap landing between the final verification and the write syscall could redirect bytes outside the workspace before any post-write check ran. The tool now fails closed on platforms without descriptor-relative writes (non-Linux, or Linux without `/proc/self/fd`): it refuses to save the attachment instead of writing through an unpinned path and reporting the breach after the fact.
