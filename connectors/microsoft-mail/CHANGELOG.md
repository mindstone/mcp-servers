...
- `download_attachment` re-verifies the attachment directory's canonical containment and pinned dev/ino identity after the destination file is created (before any bytes are written) and again after the write, so a local attacker who replaces the validated directory with a symlink mid-sequence is detected and the write fails closed with the misplaced file removed; previously the parent chain was only checked before the file was created. Node exposes no descriptor-relative opens, so a swap landing between the final verification and the write syscall itself is detected after the fact and cleaned up rather than prevented — that residual is documented on `writeFileExclusive`.
