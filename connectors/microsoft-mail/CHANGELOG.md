...
- `download_attachment` re-verifies the attachment directory's canonical containment and pinned dev/ino identity after the destination file is created (before any bytes are written) and again after the write, so a local attacker who replaces the validated directory with a symlink mid-sequence is detected and the write fails closed with the misplaced file removed; previously the parent chain was only checked before the file was created.
- `download_attachment` now pins the validated attachment directory behind an open file descriptor and addresses the create, the post-write verification, and any cleanup relative to that descriptor (`/proc/self/fd/<fd>/<name>`), so a directory swap at any point in the sequence can no longer place bytes outside `MCP_WORKSPACE_PATH` or make cleanup delete anything but the file the connector created; a rename mid-write fails closed instead of reporting a dangling `savedTo`. This descriptor-relative mechanism is only reachable through Linux `/proc/self/fd` (Node exposes no openat-style API); on other platforms the detect-and-refuse re-verification above remains the guard, with the documented residual that a swap landing between the final verification and the write syscall is detected and cleaned up after the fact rather than prevented.
