...
- `download_attachment` writes only inside `MCP_WORKSPACE_PATH` (or the OS temp directory when unset) using canonical-prefix containment on the symlink-resolved directory, rejects anything but a plain filename, and caps downloads at 25 MB. Attachment bytes are streamed from the `$value` endpoint with a hard byte cap (never fully materialized past the limit), metadata is streamed with a hard 1 MB cap for the same reason, and files are created with an exclusive open so a pre-existing file or symlink at the destination is never written through. The validated attachment directory is pinned behind an open file descriptor and the create, verification, and cleanup are addressed relative to it, so a directory swapped out mid-write can neither redirect bytes outside the workspace nor make cleanup delete the wrong file. Node exposes descriptor-relative file operations only through Linux `/proc/self/fd`, and a path-based create can always have its parent directory swapped out between validation and the write syscall, so there is no non-Linux fallback: on other platforms (or Linux without `/proc/self/fd`) the tool refuses to save the attachment rather than risk bytes landing outside the workspace. Attacker-controlled attachment fields (name, type, content type) are wrapped in untrusted-content envelopes wherever they surface, including error paths.
