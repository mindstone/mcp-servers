...
- `download_attachment` writes only inside `MCP_WORKSPACE_PATH` (or the OS temp directory when unset) using canonical-prefix containment on the symlink-resolved directory, rejects anything but a plain filename, and caps downloads at 25 MB. Attachment bytes are streamed from the `$value` endpoint with a hard byte cap (never fully materialized past the limit), metadata is streamed with a hard 1 MB cap for the same reason, and files are created with an exclusive open so a pre-existing file or symlink at the destination is never written through. On Linux the validated attachment directory is additionally pinned behind an open file descriptor and the create, verification, and cleanup are addressed relative to it, so a directory swapped out mid-write can neither redirect bytes outside the workspace nor make cleanup delete the wrong file (on other platforms, where Node cannot pin ancestor directories, the directory is re-verified after create and after write and a detected swap fails closed and is cleaned up). Attacker-controlled attachment fields (name, type, content type) are wrapped in untrusted-content envelopes wherever they surface, including error paths.
