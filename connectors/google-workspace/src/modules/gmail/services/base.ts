...
export class GmailService extends BaseGoogleService<ReturnType<typeof google.gmail>> {
  ...
  async getMessage(email: string, messageId: string) {
    await this.getGmailClient(email);
    return this.emailService.getMessage(email, messageId);
  }

  async getThread(email: string, threadId: string, options?: { maxMessages?: number; offset?: number; includeBody?: boolean }) {
    await this.getGmailClient(email);
    return this.emailService.getThread(email, threadId, options);
  }
  ...
  async trashMessage(email: string, messageId: string): Promise<void> {
    await this.getGmailClient(email);
    return this.emailService.trashMessage(email, messageId);
  }

  async untrashMessage(email: string, messageId: string): Promise<void> {
    await this.getGmailClient(email);
    return this.emailService.untrashMessage(email, messageId);
  }
