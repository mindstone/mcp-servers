...
export class SearchService {
  ...
  buildSearchQuery(criteria: SearchCriteria = {}): string {
    ...
    // Handle content - pass as raw Gmail query (supports full operator syntax)
    // The schema describes this field as "Complex Gmail search query with full operator support"
    // so we don't quote it - users/LLMs can add quotes for phrase search if needed
    if (criteria.content) {
