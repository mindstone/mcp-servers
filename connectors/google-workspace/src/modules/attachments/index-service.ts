...
export class AttachmentIndexService {
  ...
  addAttachment(messageId: string, attachment: {
    ...
    // Use attachmentId in key to avoid collision when same filename appears multiple times
    const key = `${messageId}_${attachment.id}`;
    this.index.set(key, {
      messageId,
   ...
   * Searches by filename since that's what callers typically have
   */
  getMetadata(messageId: string, filename: string): AttachmentMetadataInternal | undefined {
    // Search through entries to find matching messageId and filename
    for (const [, metadata] of this.index.entries()) {
      if (metadata.messageId === messageId && metadata.filename === filename) {
        if (this.isExpired(metadata)) {
          const key = `${messageId}_${metadata.originalId}`;
          this.index.delete(key);
          return undefined;
        }
        return metadata;
  ...
  /**
   * Retrieve attachment metadata by message ID and attachment ID (preferred)
   */
  getMetadataById(messageId: string, attachmentId: string): AttachmentMetadataInternal | undefined {
