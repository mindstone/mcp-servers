...
/**
 * Common arguments for all workspace tools
 * @property email - The Google Workspace account email address (optional - defaults to instance account)
 */
...
// Calendar Types
/**
 * Parameters for listing calendars the user has access to
 * @property email - The user's email address for authentication
 */
export interface ListCalendarsArgs extends BaseToolArguments {
  // No additional parameters - just uses email from base
}
 ...
 * @property calendarId - Calendar ID to read events from (defaults to 'primary')
 */
...
export interface ManageAttachmentParams extends BaseToolArguments {
  ...
  messageId: string;
  filename: string;
  mimeType?: string;  // MIME type for upload action
