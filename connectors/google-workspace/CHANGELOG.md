...
- `update_workspace_vacation_responder` now rejects a non-boolean `clear_end_time` / `contacts_only` / `domain_only` (and `send_workspace_email` a non-boolean `is_html`) with `InvalidParams` instead of silently coercing a wrong-typed value (e.g. the string `"true"`) to absent.
...
- Path-based Gmail attachment reads no longer have a check-then-use race window: after the workspace-containment check the file's identity (device + inode) is pinned, the file is opened once (with `O_NOFOLLOW` where supported), and the opened descriptor's identity is re-verified before the read — so swapping either the final path component or an intermediate directory for an out-of-workspace symlink between check and read fails closed, and no Gmail request is made.
